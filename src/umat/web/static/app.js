...
function renderLiveAndroidSession(content, workflow) {
  ...
  const operationTitle = node("strong", "mono", "No manual operation has run yet."); const operationOutput = node("pre", "android-command-output", "Select a tool to see its result."); const operationDownload = node("a", "btn btn-small hidden", "Download captured screenshot"); append(operation, operationTitle, operationOutput, operationDownload);
  ...
  function resultText(result) { const clean = { ...result }; if (clean.image_base64) clean.screenshot = "Captured and displayed on the live device"; delete clean.image_base64; return JSON.stringify(clean, null, 2); }
  function downloadScreenshot(encoded) { operationDownload.href = `data:image/png;base64,${encoded}`; operationDownload.download = `umat-android-${runId}-${new Date().toISOString().replaceAll(":", "-")}.png`; operationDownload.classList.remove("hidden"); operationDownload.click(); }
