...
| Phase 2 shared C2 service | Implemented with runtime gates | Isolated executor, input/result validation, adapters, recovery, and Windows/Android inputs exist. The executable runtime is pinned to reachable upstream schema-v1.3 commit `bf1f275` as effective runtime `bf1f275-umat.2`. It includes upstream universal GeoLite2 enrichment, allowlist-safe evidence, richer object/process context, normalized network modes, shared-hosting suppression, destination-safe parsing, bounded-memory PCAP hashing, and a complete UMAT-packaged, dual-digest-locked ThreatFox archive that seeds 47,832 scoped indicators. |
...
networking; see the [network architecture and real-egress target](network-architecture.md).
Operators reproducing the sacrificial gateway should follow the
[AWS egress gateway runbook](aws-egress-gateway.md); it documents the exact local contract,
...
- Failure recovery, evidence handling/retention, incident response, key custody, upgrades,
  ...
  [the Phase 6 operations runbook](phase6-operations.md).
...
3. Keep `isolated_simulated` selected for the current qualified malware baseline. Enable **C2
   ...
   [the network guide](network-architecture.md) is deployed and authorized.
...
- [Phase 2 shared C2](phase2-c2.md)
- [Malware analysis network architecture](network-architecture.md)
- [AWS sacrificial egress gateway runbook](aws-egress-gateway.md)
- [Phase 3 Windows/CAPE](phase3-windows.md)
- [Phase 4 unified UI and reports](phase4-unified-ui.md)
- [Phase 5 Android/MobSF](../deployment/android/README.md)
- [Phase 5.5 hardening and runtime gates](phase55-hardening.md)
