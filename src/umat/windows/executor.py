...
class WindowsExecutor:
    def __init__(self, umat_url: str, state_path: Path, cape: CapeClient, handoff_root: Path, schema_root: Path, work_root: Path) -> None:
    ...
    def enroll(self, token: str, name: str) -> None:
        ...
        response = self.client.post("/api/internal/v1/executors/register", json={"enrollment_token": token, "name": name, "public_key": base64.b64encode(public).decode(), "metadata": {"implementation": "umat-windows-executor", "cape_only": True}})
        response.raise_for_status()
        registered = response.json()
        self.state = {"executor_id": registered["executor_id"], "credential": registered["credential"], "private_key": base64.b64encode(raw).decode(), "name": name}
        self.state_path.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
        self.state_path.write_text(json.dumps(self.state, indent=2) + "\n")
        self.state_path.chmod(0o600)
        capabilities = self.client.post("/api/internal/v1/executors/capabilities", headers=self.auth(), json={"schema_version": "1.0", "runtime_identity": "winstdt@7bc74765e9d38d7ba6df3f2115db67761cb4cbd8", "supported_stage_types": ["platform_analysis"], "capabilities": {"platforms": ["windows"], "cape_native": True, "vm_profile_management": True, "handoff_schema": "1.0"}})
    ...
    def signed(self, method: str, path: str, body: dict[str, Any], lease: str) -> dict[str, str]:
        ...
        return self.auth() | {"X-UMAT-Timestamp": timestamp, "X-UMAT-Nonce": nonce, "Idempotency-Key": key, "X-UMAT-Signature": base64.b64encode(self.private_key.sign(message)).decode(), "X-UMAT-Lease-Token": lease}
    ...
    def process_profile_operation(self) -> bool:
        response = self.client.post("/api/internal/v1/executors/windows/profile-operations/claim", headers=self.auth())
        response.raise_for_status()
        ...
        body = {"operation_id": operation["operation_id"], "success": success, "native_operation_id": native_id, "cape_machine_label": label, "detail": detail}
        path = f"/api/internal/v1/executors/windows/profile-operations/{operation['operation_id']}/complete"
        completed = self.client.post(path, json=body, headers=self.signed("POST", path, body, operation["lease_token"]))
        completed.raise_for_status()
        return True

    def process_stage(self, poll_seconds: float) -> bool:
        response = self.client.post("/api/internal/v1/executors/claim", headers=self.auth(), json={"stage_types": ["platform_analysis"], "platforms": ["windows"]})
        response.raise_for_status()
        if response.content in {b"", b"null"}:
            return False
        claim = response.json()
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix=f"umat-windows-{claim['analysis_run_id']}-", dir=self.work_root) as temporary:
                ...
                if recovered:
                    ...
                    native = common | {"task_type": "cape", "native_task_id": str(task_id), "recovery_metadata": {"analysis_run_id": claim["analysis_run_id"], "profile": claim["execution_configuration"]}}
                    self.mutate(f"/api/internal/v1/stages/{claim['stage_id']}/native-task", native, claim["lease_token"]).raise_for_status()
            ...
            complete = common | {"outcome": "completed", "detail": "WinST/DT bundle validated and registered"}
            self.mutate(f"/api/internal/v1/stages/{claim['stage_id']}/complete", complete, claim["lease_token"]).raise_for_status()
    ...
    def _download_sample(self, claim: dict[str, Any], workspace: Path) -> Path:
        path, body = claim["sample_download_path"], {"lease_id": claim["lease_id"], "attempt_id": claim["attempt_id"], "sample": True}
        destination, digest = workspace / "sample.bin", hashlib.sha256()
        with self.client.stream("GET", path, params={"lease_id": claim["lease_id"], "attempt_id": claim["attempt_id"]}, headers=self.signed("GET", path, body, claim["lease_token"])) as response:
            response.raise_for_status()
    ...
    def _wait(self, claim: dict[str, Any], common: dict[str, Any], task_id: int, poll_seconds: float) -> dict[str, Any]:
        while True:
            ...
            if value.get("status") in {"reported", "completed", "failed_analysis", "failed_processing"}:
                return value
    ...
    def _upload(self, claim: dict[str, Any], source_path: Path, kind: str, media_type: str) -> None:
        common = {"lease_id": claim["lease_id"], "attempt_id": claim["attempt_id"]}
        envelope = common | {"kind": kind, "sha256": sha256_file(source_path), "size_bytes": source_path.stat().st_size, "media_type": media_type, "access_tier": "analyst", "bundle_id": None}
        path = f"/api/internal/v1/stages/{claim['stage_id']}/artifacts"
        with source_path.open("rb") as source:
            response = self.client.post(path, data={"envelope": json.dumps(envelope)}, files={"file": (f"{kind}.bin", source, media_type)}, headers=self.signed("POST", path, envelope, claim["lease_token"]))
        response.raise_for_status()
