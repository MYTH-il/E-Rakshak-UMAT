...
async def signed_request(
    *,
    ...
    request_hash = verify_executor_signature(executor, request, body, timestamp, nonce, idempotency_key, signature)
    previous_key = await db.scalar(select(ExecutorRequest).where(ExecutorRequest.executor_id == executor.id, ExecutorRequest.idempotency_key == idempotency_key))
    if previous_key:
        if previous_key.request_hash != request_hash:
            raise HTTPException(status.HTTP_409_CONFLICT, "idempotency key reused with different request")
        return previous_key, True
    previous_nonce = await db.scalar(select(ExecutorRequest).where(ExecutorRequest.executor_id == executor.id, ExecutorRequest.nonce == nonce))
    if previous_nonce:
        raise HTTPException(status.HTTP_409_CONFLICT, "executor nonce already used")
    record = ExecutorRequest(executor_id=executor.id, idempotency_key=idempotency_key, nonce=nonce, request_hash=request_hash)
    db.add(record)
    await db.flush()
    return record, False
...
async def validate_lease(
    ...
    *,
    ...
    if attempt.stage_id != stage.id or attempt.executor_id != executor.id or lease.attempt_id != attempt.id or lease.executor_id != executor.id or lease.stage_id != stage.id:
...
@router.post("/executors/register", response_model=RegisterExecutorResponse, status_code=status.HTTP_201_CREATED)
async def register_executor(body: RegisterExecutorRequest, db: AsyncSession = Depends(get_db)) -> RegisterExecutorResponse:
    now = datetime.now(timezone.utc)
    enrollment = await db.scalar(select(ExecutorEnrollmentToken).where(ExecutorEnrollmentToken.token_hash == token_hash(body.enrollment_token)).with_for_update())
    ...
    try:
        ...
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "public_key must be a base64 Ed25519 public key") from exc
    executor = Executor(name=body.name, executor_type=enrollment.executor_type, status=ExecutorStatus.ACTIVE, public_key=public_key, supported_stage_types=enrollment.scopes, metadata_json=body.metadata)
    db.add(executor)
    enrollment.used_at = now
    credential_raw = random_token(48)
    await db.flush()
    db.add(ExecutorCredential(executor_id=executor.id, token_hash=token_hash(credential_raw), scopes=enrollment.scopes))
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="executor.registered", target_type="executor", target_id=str(executor.id), payload={"name": executor.name, "type": executor.executor_type})
    await db.commit()
...
async def publish_capabilities(body: CapabilityRequest, executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> None:
    ...
    db.add(BackendCapabilitySnapshot(executor_id=executor.id, runtime_identity=body.runtime_identity, schema_version=body.schema_version, capabilities=body.capabilities))
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="executor.capabilities_published", target_type="executor", target_id=str(executor.id), payload={"stage_types": body.supported_stage_types, "runtime": body.runtime_identity})
    await db.commit()
...
async def claim_windows_profile_operation(
    ...
    if executor.executor_type != "windows" or "platform_analysis" not in executor.supported_stage_types:
    ...
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="windows_profile.operation_leased", target_type="windows_profile_operation", target_id=str(operation.id), payload={"operation": operation.operation_type.value})
    await db.commit()
    return {
...
async def complete_windows_profile_operation(
    ...
    timestamp, nonce, key, signature, lease_token = x_umat_timestamp, x_umat_nonce, idempotency_key, x_umat_signature, x_umat_lease_token
    if body.operation_id != operation_id:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "operation identity mismatch")
    record, replay = await signed_request(db=db, executor=executor, request=request, body=body.model_dump(mode="json"), timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature)
    ...
    if operation.state != WindowsProfileOperationState.LEASED or operation.lease_expires_at is None or operation.lease_expires_at <= now or token_hash(lease_token) != operation.lease_token_hash:
    ...
    operation.state = WindowsProfileOperationState.COMPLETED if body.success else WindowsProfileOperationState.FAILED
    ...
    response = {"operation_id": str(operation.id), "state": operation.state.value, "profile_state": profile.state.value}
    record.response_json = response
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="windows_profile.operation_completed" if body.success else "windows_profile.operation_failed", target_type="windows_profile_operation", target_id=str(operation.id), payload=response)
    await db.commit()
    return response
...
async def expire_attempt(
    ...
    if run and stage.stage_type == StageType.PLATFORM_ANALYSIS:
        ...
        run.status = RunStatus.TERMINAL
        run.result = RunResult.PARTIAL if stage.stage_type == StageType.C2_ANALYSIS else RunResult.INCONCLUSIVE
...
async def claim_stage(body: ClaimRequest, executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> ClaimResponse | None:
    allowed = set(body.stage_types).intersection(executor.supported_stage_types)
    if not allowed:
        raise HTTPException(status.HTTP_403_FORBIDDEN, "no requested stage type is in executor scope")
    ...
    advertised_platforms = {
        str(value) for value in capabilities.get("platforms", [])
    }
    ...
    if advertised_platforms:
        ...
        if requested_platforms and not compatible_platforms:
            await db.commit()
            return None
    if StageType.C2_ANALYSIS.value in allowed and capabilities.get(
    ...
    stage_query = (
        ...
        .where(AnalysisStage.state == StageState.QUEUED, AnalysisStage.stage_type.in_(allowed), (AnalysisStage.next_attempt_at.is_(None)) | (AnalysisStage.next_attempt_at <= now))
    ...
    if not stage:
        await db.commit()
        return None
    count = await db.scalar(select(func.count(AnalysisAttempt.id)).where(AnalysisAttempt.stage_id == stage.id)) or 0
    attempt = AnalysisAttempt(stage_id=stage.id, executor_id=executor.id, attempt_number=count + 1, state=AttemptState.LEASED)
    db.add(attempt)
    await db.flush()
    lease_raw = random_token(32)
    expiry = now + timedelta(seconds=get_settings().lease_ttl_seconds)
    lease = ExecutorLease(stage_id=stage.id, attempt_id=attempt.id, executor_id=executor.id, token_hash=token_hash(lease_raw), expires_at=expiry)
    db.add(lease)
    ...
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="stage.leased", target_type="analysis_stage", target_id=str(stage.id), payload={"attempt_id": str(attempt.id), "expires_at": expiry.isoformat()})
    await db.commit()
    return ClaimResponse(stage_id=stage.id, attempt_id=attempt.id, analysis_run_id=run.id, stage_type=stage.stage_type.value, platform=run.platform.value, sample_sha256=submission.sample_sha256, lease_id=lease.id, lease_token=lease_raw, lease_expires_at=expiry, timeout_seconds=stage.timeout_seconds, input_artifacts=input_artifacts, sample_download_path=f"/api/internal/v1/stages/{stage.id}/sample" if stage.stage_type == StageType.PLATFORM_ANALYSIS else None, execution_configuration=configuration.profile_snapshot if configuration else {}, recovered_native_task={"task_type": existing_native.task_type, "native_task_id": existing_native.native_task_id, "recovery_metadata": existing_native.recovery_metadata} if existing_native else None)


async def verify_stage_mutation(
    *, db: AsyncSession, executor: Executor, request: Request, stage_id: UUID, body: dict[str, Any],
    timestamp: str, nonce: str, idempotency_key: str, signature: str, lease_token: str,
) -> tuple[AnalysisStage, AnalysisAttempt, ExecutorLease, ExecutorRequest, bool]:
    record, replay = await signed_request(db=db, executor=executor, request=request, body=body, timestamp=timestamp, nonce=nonce, idempotency_key=idempotency_key, signature=signature)
    stage, attempt, lease = await validate_lease(db, executor, stage_id, UUID(str(body["lease_id"])), UUID(str(body["attempt_id"])), lease_token, allow_expired=replay)
...
async def download_stage_sample(
    ...
    stage, _, _, record, replay = await verify_stage_mutation(
        db=db, executor=executor, request=request, stage_id=stage_id, body=body,
        timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature,
        lease_token=lease_token,
    )
    ...
    try:
        ...
        await append_audit(db, actor_type="system", actor_id=None, action="sample.integrity_failed", target_type="sample", target_id=sample.sha256)
        await db.commit()
        raise HTTPException(status.HTTP_409_CONFLICT, "sample integrity verification failed") from exc
    if not replay:
        record.response_json = {"sample_sha256": sample.sha256}
        await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="sample.downloaded_by_executor", target_type="sample", target_id=sample.sha256, payload={"stage_id": str(stage.id)})
        await db.commit()
    return FileResponse(path, media_type="application/octet-stream", filename=f"sample-{sample.sha256}", headers={"X-Content-Type-Options": "nosniff", "Cache-Control": "no-store"}, content_disposition_type="attachment")
...
async def heartbeat(stage_id: UUID, body: HeartbeatRequest, request: Request, headers: tuple[str, str, str, str, str] = Depends(mutation_headers), executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    timestamp, nonce, key, signature, lease_token = headers
    stage, attempt, lease, record, replay = await verify_stage_mutation(db=db, executor=executor, request=request, stage_id=stage_id, body=body.model_dump(mode="json"), timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature, lease_token=lease_token)
    if replay and record.response_json:
        return record.response_json
    now = datetime.now(timezone.utc)
    run = await db.get(AnalysisRun, stage.analysis_run_id)
...
async def native_task(stage_id: UUID, body: NativeTaskRequest, request: Request, headers: tuple[str, str, str, str, str] = Depends(mutation_headers), executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    timestamp, nonce, key, signature, lease_token = headers
    _, _, _, record, replay = await verify_stage_mutation(db=db, executor=executor, request=request, stage_id=stage_id, body=body.model_dump(mode="json"), timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature, lease_token=lease_token)
    if replay and record.response_json:
        return record.response_json
    ...
    if existing:
        ...
        await db.commit()
        return response
    task = BackendTask(stage_id=stage_id, attempt_id=body.attempt_id, executor_id=executor.id, task_type=body.task_type, native_task_id=body.native_task_id, recovery_metadata=body.recovery_metadata)
    db.add(task)
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="native_task.recorded", target_type="analysis_stage", target_id=str(stage_id), payload={"task_type": body.task_type, "native_task_id": body.native_task_id})
...
async def upload_artifact(
    ...
    try:
        ...
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "invalid artifact envelope") from exc
    body = metadata.model_dump(mode="json")
    stage, _, _, record, replay = await verify_stage_mutation(db=db, executor=executor, request=request, stage_id=stage_id, body=body, timestamp=x_umat_timestamp, nonce=x_umat_nonce, idempotency_key=idempotency_key, signature=x_umat_signature, lease_token=x_umat_lease_token)
    if replay and record.response_json:
        return record.response_json
    ...
    try:
        ...
        await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="artifact.rejected", target_type="analysis_stage", target_id=str(stage_id), payload={"reason": str(exc)})
        await db.commit()
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, str(exc)) from exc
    run = await db.get(AnalysisRun, stage.analysis_run_id)
    artifact = Artifact(analysis_run_id=stage.analysis_run_id, stage_id=stage.id, attempt_id=metadata.attempt_id, kind=metadata.kind, sha256=metadata.sha256, size_bytes=metadata.size_bytes, media_type=metadata.media_type, object_key=stored.object_key, access_tier=AccessTier(metadata.access_tier), bundle_id=metadata.bundle_id)
    db.add(artifact)
    await db.flush()
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="artifact.registered", target_type="artifact", target_id=str(artifact.id), payload={"run_id": str(run.id) if run else None, "sha256": artifact.sha256, "kind": artifact.kind})
...
async def create_next_stage(db: AsyncSession, run: AnalysisRun, stage_type: StageType) -> AnalysisStage:
    existing = await db.scalar(select(AnalysisStage).where(AnalysisStage.analysis_run_id == run.id, AnalysisStage.stage_type == stage_type))
    if existing:
        return existing
    max_attempts, timeout_seconds = get_settings().policy_for_stage(stage_type.value)
    stage = AnalysisStage(analysis_run_id=run.id, stage_type=stage_type, state=StageState.QUEUED, max_attempts=max_attempts, timeout_seconds=timeout_seconds)
    db.add(stage)
    await db.flush()
    return stage
...
async def advance_workflow(db: AsyncSession, run: AnalysisRun, completed: AnalysisStage) -> None:
    if completed.stage_type == StageType.PLATFORM_ANALYSIS:
        ...
        adaptations = list((await db.scalars(select(AnalysisStage).where(AnalysisStage.analysis_run_id == run.id, AnalysisStage.stage_type.in_([StageType.PLATFORM_ADAPTATION, StageType.C2_ADAPTATION])))).all())
        if len(adaptations) == 2 and all(item.state == StageState.COMPLETED for item in adaptations):
...
async def complete_stage(stage_id: UUID, body: CompleteRequest, request: Request, headers: tuple[str, str, str, str, str] = Depends(mutation_headers), executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    timestamp, nonce, key, signature, lease_token = headers
    stage, attempt, lease, record, replay = await verify_stage_mutation(db=db, executor=executor, request=request, stage_id=stage_id, body=body.model_dump(mode="json"), timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature, lease_token=lease_token)
    if replay and record.response_json:
        return record.response_json
    ...
    if requested_state == StageState.UNSUPPORTED and stage.stage_type == StageType.PLATFORM_ANALYSIS:
        run.status = RunStatus.TERMINAL
        ...
        await advance_workflow(db, run, stage)
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="stage.completed", target_type="analysis_stage", target_id=str(stage.id), payload={"outcome": requested_state.value})
    response = {"stage_id": str(stage.id), "state": stage.state.value, "run_status": run.status.value, "run_result": run.result.value if run.result else None}
    record.response_json = response
    await db.commit()
    return response
...
async def fail_stage(stage_id: UUID, body: FailRequest, request: Request, headers: tuple[str, str, str, str, str] = Depends(mutation_headers), executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    timestamp, nonce, key, signature, lease_token = headers
    stage, attempt, lease, record, replay = await verify_stage_mutation(db=db, executor=executor, request=request, stage_id=stage_id, body=body.model_dump(mode="json"), timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature, lease_token=lease_token)
    if replay and record.response_json:
        return record.response_json
    ...
    attempt_count = await db.scalar(select(func.count(AnalysisAttempt.id)).where(AnalysisAttempt.stage_id == stage.id)) or 0
    run = await db.get(AnalysisRun, stage.analysis_run_id)
    if body.retryable and attempt_count < stage.max_attempts:
        ...
        if run and stage.stage_type == StageType.PLATFORM_ANALYSIS:
            ...
            run.status = RunStatus.TERMINAL
            run.result = RunResult.PARTIAL if stage.stage_type == StageType.C2_ANALYSIS else RunResult.INCONCLUSIVE
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="stage.failed", target_type="analysis_stage", target_id=str(stage.id), payload={"error_code": body.error_code, "retryable": body.retryable})
    response = {"stage_id": str(stage.id), "state": stage.state.value, "retry_at": stage.next_attempt_at.isoformat() if stage.next_attempt_at else None}
    record.response_json = response
    await db.commit()
    return response
...
async def cancellation_ack(stage_id: UUID, body: CancellationAckRequest, request: Request, headers: tuple[str, str, str, str, str] = Depends(mutation_headers), executor: Executor = Depends(current_executor), db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    timestamp, nonce, key, signature, lease_token = headers
    stage, attempt, lease, record, replay = await verify_stage_mutation(db=db, executor=executor, request=request, stage_id=stage_id, body=body.model_dump(mode="json"), timestamp=timestamp, nonce=nonce, idempotency_key=key, signature=signature, lease_token=lease_token)
    if replay and record.response_json:
        return record.response_json
    ...
    other_active = await db.scalar(select(func.count(AnalysisStage.id)).where(AnalysisStage.analysis_run_id == run.id, AnalysisStage.id != stage.id, AnalysisStage.state.in_([StageState.LEASED, StageState.RUNNING]))) or 0
    if not other_active:
        pending = list(
            (
                await db.scalars(
        ...
        run.status = RunStatus.TERMINAL
        run.result = RunResult.CANCELLED
    await append_audit(db, actor_type="executor", actor_id=str(executor.id), action="stage.cancellation_acknowledged", target_type="analysis_stage", target_id=str(stage.id))
    response = {"stage_id": str(stage.id), "state": stage.state.value, "run_status": run.status.value}
    record.response_json = response
    await db.commit()
    return response
